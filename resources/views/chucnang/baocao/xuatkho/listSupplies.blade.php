...
@section('notifi_select')
    <div class="card-header h5">
        <b>Báo Cáo Xuất Kho Theo Vật Tư
            <form action="{{route('baocao.xuat.postSuplies')}}" method="POST"  class="d-inline">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <select class="col-md-2 ml-3 d-inline form-control"  name="sup_id" id="sup_id"  required> 
                    @foreach ($sups as $sup)
                        <option name="sup_id" value="{{$sup->id}}">{{$sup->sup_name}}</option>
                    @endforeach 
                </select>
